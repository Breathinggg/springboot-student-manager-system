...
public class ScoreController {
    ...
    public void importScore(@RequestParam("importScore") MultipartFile importScore, HttpServletResponse response) {
        ...
        try {
            ...
            for (int rowNum = 1; rowNum <= sheetAt.getLastRowNum(); rowNum++) {
                ...
                if (cell == null) {
                    errorMsg += "第" + rowNum + "行员工缺失！\n";
                    continue;
                }
                // 第1列
                cell = row.getCell(1);
                if (cell == null) {
                    errorMsg += "第" + rowNum + "行团队缺失！\n";
                    continue;
                }
                // 第2列
                cell = row.getCell(2);
                if (cell == null) {
                    errorMsg += "第" + rowNum + "行绩效缺失！\n";
                    continue;
                }
        ...
        try {
            response.setHeader("Content-Disposition",
                    "attachment;filename=" + URLEncoder.encode(
                            "score_list_sid_" + score.getStudentId() + "_cid_" + score.getStudentId() + ".xls",
                            "UTF-8"));
            ...
            XSSFSheet createSheet = xssfWorkbook.createSheet("绩效列表");
            XSSFRow createRow = createSheet.createRow(0);
            createRow.createCell(0).setCellValue("员工");
            createRow.createCell(1).setCellValue("团队");
            createRow.createCell(2).setCellValue("绩效");
    ...
    public Object getScoreStatsList(@RequestParam(value = "courseid", defaultValue = "0") Integer courseid,
        ...
        if (searchType.equals("avg")) {
            ...
            avgStringList.add("最高");
            avgStringList.add("最低");
            avgStringList.add("平均");
        ...
        for (Score sc : scoreList) {
            courseName = sc.getCourseName(); // 获取团队名
            double scoreValue = sc.getScore();// 获取绩效
            if (scoreValue < 60) {
